...
1. バックエンドサーバーを起動する
2. iOS アプリを起動し、BLE で Ray-Ban グラスとペアリングする
3. 楽譜ファイル（MusicXML）をアップロードする
4. 再生・ページ送りの操作をする
...
| レイヤー | 技術 |
|---|---|
| バックエンド | Python, FastAPI, music21 |
| 楽譜パース | music21, abcjs |
| iOS アプリ | SwiftUI, Tuist, Combine |
| グラス表示 | Meta Ray-Ban Display API |
| 通信 | BLE, WebSocket |
